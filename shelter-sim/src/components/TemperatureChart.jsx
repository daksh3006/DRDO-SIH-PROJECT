...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-md border border-slate-600 bg-slate-800 px-3 py-2 text-xs shadow-lg">
      <p className="mb-1 font-medium text-slate-300">Hour {label}:00</p>
      {payload.map((p) => (
  ...
  return (
    <div className="h-full w-full rounded-lg border border-slate-700/80 bg-slate-900/60 p-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-[10px] text-slate-500">24-hour profile</span>
      </div>
      ...
      <p className="mt-1 text-center text-[10px] text-slate-500">
        Green band indicates thermal comfort range (16–26 °C)
      </p>
    </div>
  );
}
